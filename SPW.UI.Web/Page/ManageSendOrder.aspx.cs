...
{
    public partial class ManageSendOrder : System.Web.UI.Page
    {
        ...
        private void BindData()
        {
            if (Session[this.GetType().Name + "Filter"] != null)
            {
                ...
                gdvManageOrderHQ.DataSource = DataSource.OrderByDescending(x => x.CREATE_DATE);
        ...
        private List<int> GetOrderFromDeliveryIndex(int DelOrderID) 
        {
            List<int> OrderItems = new List<int>();
            DELIVERY_INDEX objDelIndex = cmdDelIndex.Select(DelOrderID);
            foreach (var delOrder in objDelIndex.DELIVERY_ORDER)
            {
                foreach (var delOrderDetails in cmdDelOrderDetails.GetAllIncludeOrderByDeliveryOrder(delOrder.DELORDER_ID))
                {
        ...
        protected void gdvManageOrderHQ_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "DelDeliveryOrder":
                    {
                        try
                        {
                            USER objUser = Session["user"] != null ? (USER)Session["user"] : null;
                            cmdOrder.FlagStatus(GetOrderFromDeliveryIndex(DelOrderID), "20", objUser.EMPLOYEE_ID);
                            cmdDelIndex.Delete(DelOrderID);
                            BindData();
                        }
                        catch (Exception ex)
                        {
                ...
                case "ViewDeliveryOrder":
                    {
                        ...
                        Response.RedirectPermanent("~/Page/SendOrderStore6.aspx?delId="+objDelivery.DELIND_ID.ToString());
                    }
                    break;
                default :
                    break;
