...
{
    public partial class SearchUnitType : System.Web.UI.Page
    {
        private DataServiceEngine _dataServiceEngine;
        private UnitTypeService cmdUnitType;

        public List<UNIT_TYPE> DataSouce
        {
            get
            {
                var list = (List<UNIT_TYPE>)ViewState["listUnitType"];
                return list;
            }
            set
            {
                ViewState["listUnitType"] = value;
            }
        }
        ...
        {
            gridUnitType.DataSource = DataSouce;
            gridUnitType.DataBind();
        }
        ...
        protected void Page_Load(object sender, EventArgs e)// Edit for Filter
        {
            if (!Page.IsPostBack)
            {
                ...
                CreateFilterPageSelected(cmdUnitType.GetAllCount());
                ClearFilter();
                BindData();
            }
            else
        ...
        protected void btnSearch_Click(object sender, EventArgs e) // Edit for Filter
        {
            ...
            ParamItems.Insert(0, txtUnitTypeCode.Text.Trim());
            ParamItems.Insert(1, txtUnitTypeName.Text.Trim());
            Session[this.GetType().Name + "Filter"] = ParamItems;
            CreateFilterDataSource();
            BindData();
        }

        protected void gridUnitType_EditCommand(object sender, System.Web.UI.WebControls.GridViewEditEventArgs e)
        {
            Response.RedirectPermanent("ManageUnitType.aspx?id=" + gridUnitType.DataKeys[e.NewEditIndex].Values[0].ToString());
        }

        protected void gridUnitType_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            gridUnitType.PageIndex = e.NewPageIndex;
            gridUnitType.DataBind();
        }
        ...
        {
            Response.RedirectPermanent("ManageUnitType.aspx");
        }
        ...
        private void BindData()
        {
            if (Session[this.GetType().Name + "Filter"] != null)
            {
                ...
                //DataSouce = cmdUnitType.GetAllByFilterCondition((string)ParamItems[0], (string)ParamItems[1], (int)ViewState["PageIndex"], (int)ViewState["PageLimit"], ref SourceItemCount);
            ...
            {
                DataSouce = cmdUnitType.GetAllByFilter((int)ViewState["PageIndex"], (int)ViewState["PageLimit"]);
                gridUnitType.DataSource = DataSouce;
                gridUnitType.DataBind();
                PrepareButtonFilterDisplay();
            }
        ...
        protected void gridUnitType_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        protected void gridUnitType_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            try
            {
                cmdUnitType.Delete(Convert.ToInt32(gridUnitType.DataKeys[e.RowIndex].Values[0].ToString()));
