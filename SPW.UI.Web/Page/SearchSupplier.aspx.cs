...
namespace SPW.UI.Web.Page
{
    public partial class SearchSupplier : System.Web.UI.Page
    {
        ...
        public List<VENDOR> DataSouce
        {
            get
            {
                var list = (List<VENDOR>)ViewState["listSupplier"];
                return list;
            }
            set
            {
                ViewState["listSupplier"] = value;
            }
        }
        ...
        {
            gridSupplier.DataSource = DataSouce;
        ...
        protected void Page_Load(object sender, EventArgs e)// Edit for Filter
        {
            if (!Page.IsPostBack)
            {
                ...
                CreateFilterPageSelected(cmdSupplier.GetAllCount());
                ClearFilter();
                BindData();
            }
            else
        ...
        protected void btnSearch_Click(object sender, EventArgs e) // Edit for Filter
        {
            ...
            ParamItems.Insert(0, txtSupplierCode.Text.Trim());
            ParamItems.Insert(1, txtSupplierName.Text.Trim());
            Session[this.GetType().Name + "Filter"] = ParamItems;
            CreateFilterDataSource();
            BindData();
        }

        protected void gridSupplier_EditCommand(object sender, System.Web.UI.WebControls.GridViewEditEventArgs e)
        {
            Response.RedirectPermanent("ManageSupplier.aspx?id=" + gridSupplier.DataKeys[e.NewEditIndex].Values[0].ToString());
        }
        ...
        {
            Response.RedirectPermanent("ManageSupplier.aspx");
        }
        ...
        private void BindData()
        {
            if (Session[this.GetType().Name + "Filter"] != null)
            {
                ...
                DataSouce = cmdSupplier.GetAllByFilterCondition((string)ParamItems[0], (string)ParamItems[1], (int)ViewState["PageIndex"], (int)ViewState["PageLimit"], ref SourceItemCount);
            ...
            {
                DataSouce = cmdSupplier.GetAllByFilter((int)ViewState["PageIndex"], (int)ViewState["PageLimit"]);
                gridSupplier.DataSource = DataSouce;
                gridSupplier.DataBind();
                PrepareButtonFilterDisplay();
            }
        ...
        protected void gridSupplier_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            try
            {
                cmdSupplier.Delete(Convert.ToInt32(gridSupplier.DataKeys[e.RowIndex].Values[0].ToString()));
