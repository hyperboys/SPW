...
{
    public partial class SearchRawType : System.Web.UI.Page
    {
        private DataServiceEngine _dataServiceEngine;
        private RawTypeService cmdRawType;

        public List<RAW_TYPE> DataSouce
        {
            get
            {
                var list = (List<RAW_TYPE>)ViewState["listRawType"];
                return list;
            }
            set
            {
                ViewState["listRawType"] = value;
            }
        }
        ...
        {
            gridRawType.DataSource = DataSouce;
            gridRawType.DataBind();
        }
        ...
        protected void Page_Load(object sender, EventArgs e)// Edit for Filter
        {
            if (!Page.IsPostBack)
            {
                ...
                CreateFilterPageSelected(cmdRawType.GetAllCount());
                ClearFilter();
                BindData();
            }
            else
        ...
        protected void btnSearch_Click(object sender, EventArgs e) // Edit for Filter
        {
            ...
            ParamItems.Insert(0, txtRawTypeCode.Text.Trim());
            ParamItems.Insert(1, txtRawTypeName.Text.Trim());
            Session[this.GetType().Name + "Filter"] = ParamItems;
            CreateFilterDataSource();
            BindData();
        }

        protected void gridRawType_EditCommand(object sender, System.Web.UI.WebControls.GridViewEditEventArgs e)
        {
            Response.RedirectPermanent("ManageRawType.aspx?id=" + gridRawType.DataKeys[e.NewEditIndex].Values[0].ToString());
        }

        protected void gridRawType_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            gridRawType.PageIndex = e.NewPageIndex;
            gridRawType.DataBind();
        }
        ...
        {
            Response.RedirectPermanent("ManageRawType.aspx");
        }
        ...
        private void BindData()
        {
            if (Session[this.GetType().Name + "Filter"] != null)
            {
                ...
                //DataSouce = cmdRawType.GetAllByFilterCondition((string)ParamItems[0], (string)ParamItems[1], (int)ViewState["PageIndex"], (int)ViewState["PageLimit"], ref SourceItemCount);
            ...
            {
                DataSouce = cmdRawType.GetAllByFilter((int)ViewState["PageIndex"], (int)ViewState["PageLimit"]);
                gridRawType.DataSource = DataSouce;
                gridRawType.DataBind();
                PrepareButtonFilterDisplay();
            }
        ...
        protected void gridRawType_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        protected void gridRawType_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            try
            {
                cmdRawType.Delete(Convert.ToInt32(gridRawType.DataKeys[e.RowIndex].Values[0].ToString()));
