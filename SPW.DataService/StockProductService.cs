...
namespace SPW.DataService
{
    public class StockProductService : ServiceBase, IDataService<STOCK_PRODUCT_STOCK>, IService
    {
        ...
        {
            STOCK_PRODUCT_STOCK obj = Datacontext.STOCK_PRODUCT_STOCK.Where(x => x.PRODUCT_CODE.Contains(item.PRODUCT_CODE) && x.COLOR_ID == item.COLOR_ID).FirstOrDefault();
        ...
        {
            return Datacontext.STOCK_PRODUCT_STOCK.Where(x => x.PRODUCT_CODE.Contains(item.PRODUCT_CODE) && x.COLOR_ID == item.COLOR_ID).ToList();
        }
